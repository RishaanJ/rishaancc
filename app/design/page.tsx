...
const surprises = [
  ...
  { text: "the best\ninteraction\nis none.", sub: "if you can remove the step, remove the step." },
...
type ContentItem =
  | { type: "video"; url: string; span?: "wide" | "tall" | "normal" }
  | { type: "site"; url: string; label: string; description?: string; span?: "wide" | "tall" | "normal" }
  | { type: "image"; src: string; alt?: string; span?: "wide" | "tall" | "normal" }
  | { type: "channel"; url: string; name: string; image: string; span?: "wide" | "tall" | "normal" }

// ── Add your content here ──────────────────────────────────────────────────
const contentItems: ContentItem[] = [
  // Videos — paste any YouTube URL
  ...
  {type: "channel", url: "https://www.youtube.com/@mateoaaron", name: "Mateo Aaron", image: "https://yt3.googleusercontent.com/gKuiYQNZhPFMDNjcmLwRmLk-YZFcvqH_BZeYienwBqxAGhMsxIRkM6lStE97umCf2GDOFv1MBQ=s160-c-k-c0x00ffffff-no-rj"},
  {type: "channel", url: "https://www.youtube.com/@kurzgesagt", name: "Kurzgesagt – In a Nutshell", image: "https://yt3.googleusercontent.com/ytc/AIdro_n1Ribd7LwdP_qKtqWL3ZDfIgv9M1d6g78VwpHGXVR2Ir4=s160-c-k-c0x00ffffff-no-rj"},
...
const beliefs = [
  { text: "good design makes you feel something.", desc: "not just look good. when it's right, you feel it" },
  { text: "design with fluidity.", desc: "nothing should feel rigid or forced. elements breathe, transitions are natural, nothing snaps. it flows like wata" },
  { text: "design should flow.", desc: "the eye should FLOW through a layout." },
  { text: "earn every pixel.", desc: "if you can remove something and nothing is lost, remove it. space is not emptiness." },
]

const fonts = [
  { name: "Geist", family: "var(--font-geist-sans)", italic: false },
  { name: "SF Pro", family: "-apple-system, 'SF Pro Display', system-ui, sans-serif", italic: false },
  { name: "SF Rounded", family: "'SF Pro Rounded', 'SF Pro Text', -apple-system, system-ui, sans-serif", italic: false },
...
export default function DesignPage() {
  ...
  const [shuffled, setShuffled] = useState<ContentItem[]>(contentItems)
  useEffect(() => {
    setShuffled([...contentItems].sort(() => Math.random() - 0.5))
  }, [])
                ...
                if (item.type === "video") {
                  ...
                  return (
                    <motion.a
                      ...
                      className={`group relative rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-900 ${spanClass}`}
                    >
                ...
                if (item.type === "site") {
                  return (
                    <motion.a
                      ...
                      viewport={{ once: true, margin: "-20px" }}
                      transition={{ duration: 0.5, delay: i * 0.05, ease: [0.16, 1, 0.3, 1] }}
                      className={`group relative rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-4 flex flex-col justify-between hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors ${spanClass}`}
                    >
                ...
                if (item.type === "channel") {
                  return (
                    <motion.a
                      ...
                      viewport={{ once: true, margin: "-20px" }}
                      transition={{ duration: 0.5, delay: i * 0.05, ease: [0.16, 1, 0.3, 1] }}
                      className={`group relative rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 flex flex-col items-center justify-center gap-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors ${spanClass}`}
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={item.image}
